...
public class mysearch4 {
	...
	protected final String[] modules = new String[] { "transformations/smart_nursing_home.henshin" };
    ...
    protected TransformationSearchOrchestration createOrchestration(
	    ...
	    orchestration.setFitnessFunction(MySNHContext.getCurrentFitness());
...
//	    					new AggregatedFitnessComparator<TransformationSolution>()) :
	...
	protected SearchExperiment<TransformationSolution> createExperiment(
			final TransformationSearchOrchestration orchestration, final int maxEvaluations) {
	...
	public void performSearch(final String initialGraph, final int solutionLength, final int populationSize,
			final int maxEvaluations, final String algorithName) {
		TransformationSearchOrchestration orchestration = createOrchestration(initialGraph, solutionLength,
				populationSize, algorithName);
		...
		experiment.run();
		System.out
				.println(solutionLength + " Total cost: " + (double) (System.currentTimeMillis() - st) / (double) 1000);
		// analyseResultsNeeded(resultNeeded);
		// handleResults(experiment);
	}

	public static void initialization() {
		Smart_nursing_homePackage.eINSTANCE.eClass();
...
//		int levels[] = {2};
		...
		String dataPath = homePath + "/workspace/SNH";
		String caseName = "Smart Nursing Home";
		String model = "input/SmartNursingHome_small.xmi";
		...
		for (int level : levels) {
			for (boolean propagation : propagations) {
				...
				for (int i = 0; i < repeatTimes; i++) {
					// String n = new SimpleDateFormat("MM-dd-HH-mm").format(new Date()) + "_" +
					// algoName + "_prop_" + propagationRate + "_eb_" + eb + "_population_" +
					// population + "_";
					...
					try {
						...
						MySNHContext.fw = new FileWriter(file, false);
						// MySearchContext.init(propagation, propagationRate, level, goal);
						MySNHContext.init(propagation, level);
						initialization();
						mysearch4 search = new mysearch4();
						search.performSearch(model, eb, population, pb / eb, algoName);
						finalization();
						MySNHContext.fw.close();
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
